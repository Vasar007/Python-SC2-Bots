...
class CleverBot(sc2.BotAI):
    ...
    def __init__(self, use_model=False, save_train_data=False, title=1,
                 debug=False):
        self.MAX_WORKERS = 66
    ...
    async def intel(self):
        ...
        plausible_supply = self.supply_cap / self.MAX_SUPPLY_CAP
        ...
        # Plausible supply (supply / self.MAX_SUPPLY_CAP).
    ...
    async def build_pylon(self):
            ...
            if nexuses.exists:
                if self.can_afford(PYLON):
                    pylon = self.units(NEXUS).first.position.towards(
                        self.game_info.map_center, 5
                    )
                    await self.build(PYLON, near=pylon)
...
def main():
    run_game(maps.get("AbyssalReefLE"), [
                 Bot(Race.Protoss, CleverBot(use_model=False, title=1)),
                 Computer(Race.Terran, Difficulty.Medium)
             ], realtime=False)
